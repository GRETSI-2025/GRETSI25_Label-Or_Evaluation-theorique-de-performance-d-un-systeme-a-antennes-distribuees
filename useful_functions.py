...
#                              Useful functions                               #
###############################################################################
def generate_blocked_CIR(
    unblocked_cir,
    blockage_type,
    num_clusters_to_block,
    custom_mask,
    cluster_powers,
    batch_size,
    num_ofdm_symbols,
    ...
    tiled_powers = tf.tile(
        expanded_powers, [batch_size, 1, 1, 1, 1, 1, num_ofdm_symbols]
    )
    ...
    if blockage_type == "strongest_cluster_blocked":
        ...
        blockage_mask = tf.where(
            cluster_ranking < num_clusters_to_block,
        ...
        blockage_mask = tf.tile(
            blockage_mask, [batch_size, 1, 1, 1, 1, 1, num_ofdm_symbols]
        )

    else:
        raise ValueError(
            "Invalid blockage type. Choose from 'strongest_cluster_blocked', 'weakest_cluster_blocked', or 'custom'."
        )
...
def compute_marginal_cdf(
    val_threshold, corr_coeff, std_numerator, std_denominator, for_real_part=True
):
    """
    ...
    if for_real_part:
        relative_value = (
            val_threshold / std_numerator - real_part_corr / std_denominator
        )
    else:
        relative_value = (
            val_threshold / std_numerator - imag_part_corr / std_denominator
        )

    denominator_value = np.sqrt(
        (1 - np.abs(corr_coeff) ** 2) / std_denominator**2 + relative_value**2
    )
...
def corr_num_denom(symbol_d, bs_power, var_hB, var_hNB, M, ebno_base10):
    ...
    N0_over_mod_x_squared = (
        (ebno_base10**-1) * (2 * (M - 1)) / (3 * (a**2 + b**2) * np.log2(M))
    )
    ...
    denominator = np.sqrt(
        (np.sum(weighted_var_link) + N0_over_mod_x_squared) * np.sum(var_hNB * bs_power)
    )
...
def joint_pdf_ratio_complex_gaussian(
    m_x, m_y, sigma_x, sigma_y, rho, span_real, span_complex
):
    """
...
def marginal_pdf_component_ratio_centered_complex_gaussian(
    x, rho, sigma_x, sigma_y, real=True
):
    """
    ...
    term = ((x / sigma_x) - (component / sigma_y)) ** 2 + (1 - np.abs(rho) ** 2) / (
        sigma_y**2
    )
