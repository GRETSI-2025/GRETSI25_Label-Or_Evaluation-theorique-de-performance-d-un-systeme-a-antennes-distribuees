...
Power_constellation_16QAM = 10  # Constant for the 16-QAM constellation (not used directly here)
...
for i, symbol in enumerate(symboles_array):
    ...
    corr_value = useful_functions.corr_num_denom(symboles_array_non_norm[i],
                                                 array_BS_power,
                                                 array_variance_hB,
                                                 array_variance_hNB,
                                                 M,
    ...
    denom1 = mu_link_blocked * array_variance_hB  # Blocked links variance weighted by power
    denom2 = mu_link_unblocked * array_variance_hNB  # Unblocked links variance weighted by power
    denom3 = array_BS_power * array_variance_hNB  # Common denominator part for all symbols
    ...
    N0_over_mod_x_squared_d = (ebno_base10 ** -1) * (2 * (M - 1)) / (3 * ((a_d)**2 + (b_d)**2) * np.log2(M))
    ...
    sigma_r_current = np.sqrt(np.abs(symboles_array[i])**2 * (np.sum(denom1) + np.sum(denom2) + N0_over_mod_x_squared_d))
...
plt.annotate(r"$\mathbf{\rho=-0.88+j0.29}$", 
             xy=(-0.8, 0.1), 
             xytext=(-1.45, -0.5),
             arrowprops=dict(arrowstyle="->", linewidth=2),
             weight='bold', fontsize=14)

plt.annotate(r"$\mathbf{\sigma_r=1.0}$", 
             xy=(-1, 0.6), 
             xytext=(-1.45, -0.65),
             weight='bold', fontsize=14)

# Annotation for a symbol with correlation ρ = 0.57 - j0.57
plt.annotate(r"$\mathbf{\rho=0.57-j0.57}$", 
             xy=(0.4, -0.4), 
             xytext=(0.25, -0.8),
             arrowprops=dict(arrowstyle="->", linewidth=2),
             weight='bold', fontsize=14)

plt.annotate(r"$\mathbf{\sigma_r=0.51}$", 
             xy=(0.4, -0.5), 
             xytext=(0.25, -0.95),
             weight='bold', fontsize=14)

# Annotation for a symbol with correlation ρ = 0.66 + j0.66
plt.annotate(r"$\mathbf{\rho=0.66+j0.66}$", 
             xy=(1, 0.6), 
             xytext=(-1.25, 1.5),
             weight='bold', fontsize=14)

plt.annotate(r"$\mathbf{\sigma_r=1.32}$", 
             xy=(0.6, 1), 
             xytext=(-1.25, 1.35),
             arrowprops=dict(arrowstyle="->", linewidth=2),
             weight='bold', fontsize=14)
...
pdf_real = useful_functions.marginal_pdf_component_ratio_centered_complex_gaussian(x, corr_array[0], sigma_r_i[0], sigma_y=1)
pdf_real2 = useful_functions.marginal_pdf_component_ratio_centered_complex_gaussian(x, (0.5+0.5j), sigma_r_i[0], sigma_y=1)
...
x_min = 0
plt.fill_between(x, pdf_real, where=(x <= x_min), color='green', alpha=0.3, hatch='/', linewidth=4)
plt.fill_between(x, pdf_real2, where=(x <= x_min), color='yellow', alpha=0.3, hatch='/', linewidth=4)

# Annotate the plot with arrows pointing to specific points on the curves
plt.annotate(r"$\mathbf{F_{Z_I}(z_I=0,\rho_2)}$", xy=(-0.1, 0.01), xytext=(-3, 0.2),
             arrowprops=dict(arrowstyle="->", linewidth=2), weight='bold', fontsize=15)
plt.annotate(r"$\mathbf{F_{Z_I}(z_I=0,\rho_1)}$", xy=(-0.1, 0.18), xytext=(-3, 0.36),
             arrowprops=dict(arrowstyle="->", linewidth=2), weight='bold', fontsize=15)
